...
const ModuleManagementParse = (type, detail, detail1, detail2, detail3, detail4) => {
  switch(type){
		case 0:
			return "invalid";
		case 1:
		...
		case 2:
			return `Missing Module Count= ${detail}, module type = ${ModuleTypeParse((detail1>>16) & 0xffff)} , Module ID = ${detail1 & 0xffff} `;
		case 3: 
			return `Reconnected Module Count= ${detail}, module type = ${ModuleTypeParse((detail1>>16) & 0xffff)} , Module ID = ${detail1 & 0xffff} `;
		case 4:
      return `Added Module Count= ${detail}, module type = ${ModuleTypeParse((detail1>>16) & 0xffff)} , Module ID = ${detail1 & 0xffff} `;
		case 5:
      return `Confilct Module Count= ${detail}, module type = ${ModuleTypeParse((detail1>>16) & 0xffff)} , Module ID = ${detail1 & 0xffff} `;
		case 6:
...
const ParseContent = (val, detail, detail1, detail2, detail3, detail4) => {
	...
	switch(eventType) {
		case 0:
			return "invalid";
		case 1: //DI
      const diState = ((val>>20) & 0x7) === 0x1 ? 'Energized' : 'De-Energized';
      ...
      const connectionState = ((detail >>16) & 0xffff) === 1 ? 'Connected' : 'DisConnected';
      ...
      const type = (val >> 16) & 0x3;
			return ModuleManagementParse(type, detail, detail1, detail2, detail3, detail4);
	}
};
...
function LMEvents() {
  ...
  usePolling('set-event', setEvent);
  
  const type = ParseInfo(event.info);
  const content = ParseContent(event.info, event.detail, event.detail1, event.detail2, event.detail3, event.detail4);
 
  const data = {
    type : type,
    content : content
  }
  setEvents(event.concat(data));
  // const data = useMemo(
  //   () => [
  //     {
  //       index: 1,
  //       date: "2021.06.03",
  //       type: ParseInfo(event.info),
